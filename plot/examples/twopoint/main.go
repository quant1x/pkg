...
func main() {
	...
	ts1 := plot.ContinuousSeries{ //TimeSeries{
		Name:    "Time Series",
		XValues: []float64{10 * b, 20 * b, 30 * b, 40 * b, 50 * b, 60 * b, 70 * b, 80 * b},
		YValues: []float64{1.0, 2.0, 30.0, 4.0, 50.0, 6.0, 7.0, 88.0},
	}

	ts2 := plot.ContinuousSeries{ //TimeSeries{
		Style: plot.Style{
			StrokeColor: plot.GetDefaultColor(1),
		},
	...
	graph := plot.Chart{
		...
		XAxis: plot.XAxis{
			Name:           "The XAxis",
			ValueFormatter: plot.TimeMinuteValueFormatter, //TimeHourValueFormatter,
		},
	...
	err := graph.Render(plot.PNG, buffer)
	if err != nil {
		log.Fatal(err)
	}
