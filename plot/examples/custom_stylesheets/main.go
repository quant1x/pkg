...
func svgWithCustomInlineCSS(res http.ResponseWriter, _ *http.Request) {
	res.Header().Set("Content-Type", plot.ContentTypeSVG)

	// Render the CSS with custom css
	err := pieChart().Render(plot.SVGWithCSS(style, ""), res)
	if err != nil {
		fmt.Printf("Error rendering pie plot: %v\n", err)
	}
}
...
func svgWithCustomInlineCSSNonce(res http.ResponseWriter, _ *http.Request) {
	...
	err := pieChart().Render(plot.SVGWithCSS(style, nonce), res)
	if err != nil {
		fmt.Printf("Error rendering pie plot: %v\n", err)
	}
}
...
func svgWithCustomExternalCSS(res http.ResponseWriter, _ *http.Request) {
	...
	res.Header().Set("Content-Type", plot.ContentTypeSVG)
	err := pieChart().Render(plot.SVG, res)
	if err != nil {
		fmt.Printf("Error rendering pie plot: %v\n", err)
	}
}

func pieChart() plot.PieChart {
	return plot.PieChart{
		// Note that setting ClassName will cause all other inline styles to be dropped!
		Background: plot.Style{ClassName: "background"},
		Canvas: plot.Style{
			ClassName: "canvas",
		},
		Width:  512,
		Height: 512,
		Values: []plot.Value{
			{Value: 5, Label: "Blue", Style: plot.Style{ClassName: "blue"}},
			{Value: 5, Label: "Green", Style: plot.Style{ClassName: "green"}},
			{Value: 4, Label: "Gray", Style: plot.Style{ClassName: "gray"}},
		},
	}
}
