...
// GetDPI returns the dpi for the plot.
...
// GetWidth returns the plot width or the default value.
...
// GetHeight returns the plot height or the default value.
...
// Render renders the plot with the given renderer to the given io.Writer.
func (c Chart) Render(rp RendererProvider, w io.Writer) error {
	...
	Debugf(c.Log, "plot; canvas box: %v", canvasBox)
	...
	if c.hasAxes() {
		...
		Debugf(c.Log, "plot; axes adjusted canvas box: %v", canvasBox)
	...
	if c.hasAnnotationSeries() {
		...
		Debugf(c.Log, "plot; annotation adjusted canvas box: %v", canvasBox)
	}
...
func (c Chart) checkHasVisibleSeries() error {
	...
	return fmt.Errorf("plot render; must have (1) visible series")
}
...
func (c Chart) getAxesAdjustedCanvasBox(r Renderer, canvasBox Box, xr, yr, yra Range, xticks, yticks, yticksAlt []Tick) Box {
	...
	if !c.XAxis.Style.Hidden {
		...
		Debugf(c.Log, "plot; x-axis measured %v", axesBounds)
		axesOuterBox = axesOuterBox.Grow(axesBounds)
	}
	if !c.YAxis.Style.Hidden {
		axesBounds := c.YAxis.Measure(r, canvasBox, yr, c.styleDefaultsAxes(), yticks)
		Debugf(c.Log, "plot; y-axis measured %v", axesBounds)
		axesOuterBox = axesOuterBox.Grow(axesBounds)
	}
	if !c.YAxisSecondary.Style.Hidden && c.hasSecondarySeries() {
		...
		Debugf(c.Log, "plot; y-axis secondary measured %v", axesBounds)
		axesOuterBox = axesOuterBox.Grow(axesBounds)
	}
...
// GetColorPalette returns the color palette for the plot.
...
// Box returns the plot bounds as a box.
